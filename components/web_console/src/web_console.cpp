...
"--flow-a:rgba(74,134,255,.58);--flow-b:rgba(166,103,255,.48);--flow-c:rgba(255,102,170,.34);"
"--flow-opacity:.58;--flow-saturation:1;--flow-speed-a:24s;--flow-speed-b:31s;--flow-speed-c:37s"
"}"
...
"background-size:42px 42px;mask-image:linear-gradient(to bottom,#000,transparent 72%);z-index:0}"
"body>header,body>main,body>footer{position:relative;z-index:1}"
".gemini-flow{position:fixed;inset:0;z-index:0;overflow:hidden;pointer-events:none;"
"opacity:var(--flow-opacity);filter:saturate(var(--flow-saturation));transition:opacity 1.2s ease,filter 1.2s ease}"
".gemini-flow i{position:absolute;display:block;width:78vmax;height:78vmax;border-radius:50%;"
"will-change:transform,opacity;background-repeat:no-repeat;mix-blend-mode:screen}"
".flow-a{left:-42vmax;top:-35vmax;background:radial-gradient(circle at 62% 58%,var(--flow-a) 0 5%,rgba(78,159,255,.3) 18%,transparent 62%);"
"animation:flowA var(--flow-speed-a) cubic-bezier(.45,.05,.55,.95) infinite alternate}"
".flow-b{right:-45vmax;top:5vh;background:radial-gradient(circle at 38% 46%,var(--flow-b) 0 7%,rgba(107,78,255,.26) 23%,transparent 64%);"
"animation:flowB var(--flow-speed-b) cubic-bezier(.45,.05,.55,.95) infinite alternate}"
".flow-c{left:10vw;bottom:-58vmax;background:radial-gradient(circle at 48% 35%,var(--flow-c) 0 6%,rgba(255,174,87,.2) 22%,transparent 60%);"
"animation:flowC var(--flow-speed-c) cubic-bezier(.45,.05,.55,.95) infinite alternate}"
"body[data-nova-state=thinking]{--flow-opacity:.84;--flow-saturation:1.25;--flow-speed-a:8s;--flow-speed-b:11s;--flow-speed-c:14s}"
"body[data-nova-state=running],body[data-nova-state=listening]{--flow-opacity:.78;--flow-saturation:1.18;--flow-speed-a:10s;--flow-speed-b:14s;--flow-speed-c:18s}"
"body[data-nova-state=success]{--flow-opacity:.76;--flow-saturation:1.12;--flow-speed-a:18s;--flow-speed-b:23s;--flow-speed-c:28s}"
"body[data-nova-state=setup],body[data-nova-state=warning]{--flow-a:rgba(255,159,71,.48);--flow-b:rgba(255,103,139,.42);--flow-c:rgba(255,209,102,.3)}"
"body[data-nova-state=error],body[data-nova-state=offline]{--flow-opacity:.24;--flow-saturation:.38;--flow-speed-a:46s;--flow-speed-b:52s;--flow-speed-c:58s}"
...
"@keyframes flowA{0%{transform:translate3d(0,0,0) scale(.9) rotate(-8deg);opacity:.72}55%{opacity:1}100%{transform:translate3d(38vw,22vh,0) scale(1.2) rotate(20deg);opacity:.82}}"
"@keyframes flowB{0%{transform:translate3d(0,-8vh,0) scale(1.08) rotate(10deg);opacity:.65}100%{transform:translate3d(-42vw,20vh,0) scale(.88) rotate(-18deg);opacity:1}}"
"@keyframes flowC{0%{transform:translate3d(-12vw,0,0) scale(.82);opacity:.58}100%{transform:translate3d(26vw,-38vh,0) scale(1.15);opacity:.9}}"
...
"@media(prefers-reduced-motion:reduce){html{scroll-behavior:auto}*,*:before,*:after{animation-duration:.001ms!important;animation-iteration-count:1!important;transition-duration:.001ms!important}.nova-core{transform:none!important}.gemini-flow{opacity:.3}}"
"</style>"
"</head>"
"<body>"
"<div class=\"gemini-flow\" aria-hidden=\"true\"><i class=\"flow-a\"></i><i class=\"flow-b\"></i><i class=\"flow-c\"></i></div>"
"<header class=\"topbar\">"
"<div class=\"logo\">"
...
" currentNovaState=state;document.body.setAttribute('data-nova-state',state);$('nova').className='nova-shell '+state;$('novaTitle').textContent=copy.title;$('novaDetail').textContent=copy.detail;$('novaActionBtn').textContent=copy.action;"
...
" opts.forEach(function(opt){var btn=document.createElement('button');btn.className='btn-success';btn.textContent=(opt.name||t('modules.unnamed'))+' (ID '+opt.id+')';btn.onclick=function(){pickDevice(opt)};actions.appendChild(btn)});root.appendChild(actions);clearNova()"
"}"
"async function pickDevice(opt){"
" var r=window._clarify;if(!r)return;"
" var id=opt.id;var script=(r.script||'').split(r.placeholder).join(JSON.stringify(opt.name||''));"
